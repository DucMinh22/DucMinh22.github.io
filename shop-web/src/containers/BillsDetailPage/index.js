...
function BillsDetail() {
  ...
  const billArray = productsbill.bills;

  // sum prices
  const total = () =>
    billArray.reduce((total, currentItem) => {
      return (
        total +
    ...
    <div>
      ...
      <div className="info">
        <div className="info-detail">
          <p>
            {t(`billDetailPage.user`)}:{" "}
            <span style={{ textTransform: "uppercase" }}>
              {productsbill.username}
            </span>
          </p>
          <p>{/* {t(`billDetailPage.total`)}: {total()} */}</p>
        </div>
        <div className="Status">
          ...
          >
            <Option value="Shipping">
              {t(`billDetailPage.select.shipping`)}
            </Option>
          ...
          >
            {t(`billDetailPage.update`)}
          </Button>
        </div>
      </div>
      <Table
        columns={columns}
        dataSource={productsbill.bills?.map((item) => ({
          ...item,
          key: item.id,
        }))}
        onChange={onChange}
