...
export default function CategoryPage() {
  ...
  const { loading, categories, products } = categoriesState;
        ...
        <Sider className="menu">
          ...
          {categories.map((item) => {
            return (
              <div
                className={`${
                  chosenCategory === Number.parseInt(item.id)
                    ? "menu__item-active"
                    : "menu__item"
                }`}
                key={item.id}
                onClick={() => handleChooseCategory(Number.parseInt(item.id))}
              >
                <EuroCircleOutlined
            ...
            <div className="optionField__item">
              ...
              >
                <Select.Option value="asc">
                  {t(`categorypage.increment`)}
                </Select.Option>
                <Select.Option value="desc">
                  {t(`categorypage.decrement`)}
                </Select.Option>
              </Select>
            </div>
            <Button onClick={handleSearch}>{t(`search`)}</Button>
          </div>
          <Content className="mainContent">{renderProducts(products)}</Content>
          <div className="loadMoreField">
            <Button onClick={handleLoadMore} disabled={sortBy && order}>
              {t(`categorypage.more`)}
            </Button>
          </div>
        </Layout>
